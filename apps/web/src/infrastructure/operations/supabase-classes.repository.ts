...
function aClase(fila: Record<string, unknown>): Clase {
  return {
    ...
    walkinSpots: numero(fila.walkin_spots),
...
function aSesion(fila: Record<string, unknown>): SesionDeClase {
  ...
  return {
    ...
    reservadas: numero(fila.reservadas),
    enEspera: numero(fila.en_espera),
    // Sin la columna (vista anterior a V3.4) los ocupados son los asistentes.
    ocupados: fila.ocupados === undefined ? numero(fila.asistentes) : numero(fila.ocupados),
    walkinSpots: numero(fila.walkin_spots),
    miReservaId: texto(fila.mi_reserva_id),
    miReservaEstado: texto(fila.mi_reserva_estado),
    miPosicion: opcional(fila.mi_posicion),
...
function filaDeClase(datos: DatosDeClase) {
  return {
    ...
    walkin_spots: datos.walkinSpots,
