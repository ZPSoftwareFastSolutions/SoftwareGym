 ...
 *
 * No hay bandeja de consultas en la plataforma: el formulario (V4.2,
 * `ContactForm`) valida en el navegador y en el servidor y abre WhatsApp con la
 * consulta ya redactada al número del gimnasio. Cuando exista una bandeja, se
 * cambia la acción y nada más.
 */
...
export function ContactSection({ contact, social, name, showForm, showMap, slug }: ContactSectionProps) {
  const details = [
    ...
    <section className="section" aria-labelledby="contacto-title">
      <div className="shell">
        ...
        <div className="mt-14 grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:gap-14">
          ...
          {showForm && (
            <Reveal delay={100}>
              <ContactForm slug={slug} gimnasio={name} />
            </Reveal>
          )}
        </div>
      </div>
