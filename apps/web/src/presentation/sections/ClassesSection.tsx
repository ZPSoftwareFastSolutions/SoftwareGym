...
import { duracionEnMinutos, NOMBRE_DE_NIVEL_DE_CLASE } from '@core/domain/catalog/classes';
    ...
    <section className="relative w-full py-16 md:py-24">
      <div className="shell relative z-20 mx-auto w-full max-w-7xl">
        {!hideTitle && (
          <div className="flex flex-col md:flex-row items-end justify-between gap-8 mb-16">
            ...
            <Reveal delay={100} className="hidden md:block">
              <LinkButton href={tenantHref(slug, 'clases')} variant="outline" icon="arrowRight" className="text-white border-white/20 hover:bg-white hover:text-black">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {clases.map((clase, i) => {
            // La duración sale del horario oficial, no de un valor fijo: Baile
            // urbano y Heels duran 90 minutos. Si ninguna franja publica su
            // cierre, la etiqueta no se muestra en vez de inventar una cifra.
            const duracion = clase.horarios.map(duracionEnMinutos).find((m): m is number => m !== null);
            let emoji = '⚡';
        ...
        <div className="mt-10 md:hidden flex justify-center">
          <LinkButton href={tenantHref(slug, 'clases')} variant="outline" className="text-white border-white/20 hover:bg-white hover:text-black">
            Ver horario completo
          </LinkButton>
        </div>
      </div>
