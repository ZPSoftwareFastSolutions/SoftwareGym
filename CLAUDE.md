...
>
> - **Última actualización:** 2026-09-25 · rama `miticogym-v5`: auditoría de calidad, avisos legales y accesibilidad (ver §9).
> - Antes: 2026-09-22 · rama `miticogym-v4`: logotipo oficial
...
| Fecha | Qué pasó |
|---|---|
| 2026-09-25 | **`miticogym-v5`** (desde v4): auditoría de calidad y cumplimiento. **Legal:** `/legal/privacidad`, `terminos`, `cookies` y `reembolsos` generados desde la configuración (`core/domain/legal`), enlazados en el pie y en el sitemap; citan solo normas verificables (CPE arts. 21 y 130, Ley 453) y no inventan razón social, NIT ni condiciones de reembolso. **Cookies:** el mapa de Google se carga solo al pulsar «Ver mapa» (`MapaBajoDemanda`), así que el sitio no instala cookies sin acción del usuario y no necesita banner. **Formulario:** fuera el correo (dato innecesario), consentimiento explícito con enlace a privacidad, `noopener` en WhatsApp y aviso falso de «versión de demostración» retirado. **Contenido:** eliminadas cifras inventadas de instalaciones (m², equipos), insignias «Más popular/Más elegido», «16 h abierto cada día», «una sola membresía», «incluidos en tu membresía», «Sin contratos forzosos» y la razón social «S.R.L.» no confirmada. **Accesibilidad:** contraste AA en todo el texto (white/40 → /70, prestaciones no incluidas legibles y anunciadas), acordeón con foco visible y `aria-expanded`, botones de alternancia con `aria-pressed`, emojis ocultos a lectores, jerarquía de encabezados sin saltos, h1 en Servicios. **SEO:** canónica por página (todas apuntaban a la portada), `metadataBase`, JSON-LD `HealthClub` solo con datos verificados, imagen Open Graph, robots sin el resto de `/acceso`. 35 pruebas; build de 14 páginas; auditoría automática sin incidencias en 14 páginas a 1280 y 375 px. |
