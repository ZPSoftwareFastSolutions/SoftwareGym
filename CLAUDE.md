...
**Estado (2026-09-22, tarjeta de QR):** typecheck limpio · **379 pruebas** (35 nuevas: 28 de geometría y 7 de dibujo)
· `npm run build` correcto · commit `af2d473` en `origin/goldgym-v5` · **desplegado a producción**
(`dpl_7bZv7js1wcgPbyfhKSCzbx2e3H1k`, verificado sobre el dominio: públicas 200, panel 307, ruta inventada 404).

> **El proyecto Vercel `gold-gym` ya NO está conectado a GitHub**: empujar a la rama no dispara build. Se despliega
> a mano con `npx vercel deploy --prod` **desde `apps/web`** (es la carpeta que sube el CLI; los despliegues
> anteriores con `githubCommitSha` son de cuando sí había integración).
